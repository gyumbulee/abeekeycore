...
<head>
    <meta charset="utf-8">
    <title>Domain renewal reminder from Abeekey</title>
</head>
    ...
    <div style="max-width: 480px; margin: 0 auto; background: #FFFFFF; border-radius: 12px; overflow: hidden; border: 1px solid #E2E8F0;">
        <div style="background: linear-gradient(135deg, #0B1F3A, #153E75); padding: 28px;">
            <h2 style="color: #FFFFFF; margin: 0;">
                {{ $order->domain_name }}{{ $order->tld }} {{ $daysRemaining <= 1 ? 'expires tomorrow' : "expires in {$daysRemaining} days" }}
            </h2>
        </div>
        <div style="padding: 28px;">
            ...
            <p>
                ...
                <strong>{{ $order->domain_name }}{{ $order->tld }}</strong>
                is set to expire on <strong>{{ $order->expires_at->format('d M Y') }}</strong>.
                Renewing before then keeps the domain — and any website or email
                built on it — from going offline.
            </p>
            ...
            <p style="color: #64748B; font-size: 14px; margin-top: 24px;">
                Need help renewing, or have questions? Reach us at
